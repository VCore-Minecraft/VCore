...
public class SetGameModeUpdate extends Update {
    public SetGameModeUpdate(@NotNull UUID uuid) {
        super(uuid);
    }

    @Override
    public List<Class<?>> instructionDataTypes() {
        return List.of(UUID.class, PlayerGameMode.class);
    }
    ...
    protected @NotNull UpdateCompletion executeUpdate(TransmittedData transmittedData) {
        ...
        var gameMode = transmittedData.getObject(1, PlayerGameMode.class);
        ...
        return UpdateCompletion.fromBoolean(gameServerPlatform.setGameMode(targetUUID, gameMode).join());
    }
}
